...
if is_sudoer; then
  ...
  wget "${GO_URL}go${version_build}.linux-arm64.tar.gz"

  ## Extract and install the downloaded version
  if [ -f "go${version_build}.linux-arm64.tar.gz" ]; then
    sudo rm -rf "${pkg_path}" && sudo tar -xzvf "go${version_build}.linux-arm64.tar.gz" -C "${LOCAL_PATH}"
